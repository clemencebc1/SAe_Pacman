...
def Joueur(couleur, nom, nb_points, nb_faux_mvt, pos_pacman, pos_fantome, objets):
    ...
    """
    return {'couleur': couleur, 'nom': nom, 'nb_points': nb_points, 'nb_faux_mvt': nb_faux_mvt, 'pos_pacman': pos_pacman, 'pos_fantome': pos_fantome, 'objets': objets}
...
def joueur_from_str(description):
    ...
    objets = {'$': int(infos[7]), '@': int(infos[8]), '~': int(infos[9])}
    pos_pacman = (int(infos[3]), int(infos[4]))
    pos_fantome = (int(infos[5]), int(infos[6]))

    return Joueur(infos[0], infos[-1], int(infos[1]), int(infos[2]), pos_pacman, pos_fantome, objets)
    

print(joueur_from_str('A;2;0;1;1;2;2;4;2;0;B'))
...
def get_objets(joueur):
    ...
    Returns:
        list(int): la liste des objets possédés pint(infos[5]), int(infos[6])ar le joueur
    """
    objets_possedees = []
    for cle in joueur['objets']:
        if joueur['objets'][cle] > 0:
            objets_possedees.append(cle)
    return objets_possedees
...
def maj_duree(joueur):
    ...
    Args:
        joueur (dict): le joueur considéré
    """
    for objet in get_objets(joueur):
